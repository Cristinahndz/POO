// Menu.js
//envia las ordenes hechas en el menu hacia firestore
...
const Menu = () => {
  const [menuItems, setMenuItems] = useState(exampleMenu); //almacena la lista de elementos dell menu
  const [currentOrder, setCurrentOrder] = useState({});//guarda las ordenes actuales
  const [currentTotal, setCurrentTotal] = useState(0);//guarda el total actual del pedido
  const [searchResults, setSearchResults] = useState([]);//almacena los resultados de la busqueda en el menu 
  const [presentItems, setPresentItems] = useState({});//mantiene un registro de los elementos presentes en el menu
  ...
  const confirmOrder = async () => {//se encarga de confirmar la orden, agrega a firestore 
    //verificando que haya un usuario autenticado primero
    try {
  ...
  const searchMenu = (searchInput) => { //maneja la busqueda en el menu
